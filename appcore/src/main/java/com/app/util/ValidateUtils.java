...
public class ValidateUtils {
	...
	/** 手机 */
	private static final String V_MOBILE = "^((\\+86)|(86))?1[3|4|5|7|8][0-9]\\d{8}$";
	// private static final String V_MOBILE =
	...
	// private static final String V_TEL =
	// "^(([0\\+]\\d{2,3}-)?(0\\d{2,3})-)?(\\d{7,8})(-(\\d{3,}))?$";
	private static final String V_TEL = "^(([0\\+]\\d{2,3}-{0,1})?(0\\d{2,3})-{0,1})?(\\d{7,8})(-{0,1}(\\d{3,}))?$";;

	/** 用来用户注册。匹配由数字、26个英文字母或者下划线组成的字符串 */
	// private static final String V_USERNAME = "^\\w{0,50}$";//名字不作任何限制
	...
	/** 密码 */
	private static final String V_PASSWORD = "^[a-zA-Z0-9]{6,12}$";

	/** 验证密码(数字和英文同时存在) */
	private static final String V_PASSWORD_REG = "^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,50}$";

	/** 验证账号名(数字或英文) */
	private static final String V_ACCOUNT_REG = "[A-Za-z0-9]+";
	 ...
	 * @return 如果是符合格式的字符串,返回 <b>true </b>,否则为 <b>false </b>
	 */
	public static boolean Account_reg(String value) {
		return match(V_ACCOUNT_REG, value);
	}
